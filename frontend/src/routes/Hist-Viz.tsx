...
export default function HistViz() {
    ...
    useEffect(() => {
        ...
        if (savedState) {
            ...
            currentData.histograms.forEach((entry) => {
            ...
            setAllSelections((prev) => ({ ...prev, ...defaultSelections }));
            setIsEditing(true);
        }
    }, [currentData, nodeId, histogramStates]);
    ...
    const rows = useMemo(() => {
        ...
        for (const h of filteredHistograms) {
            const key = perspective === 'event' ? h.event_type : h.object_type;
    ...
    }, [currentData, sortMode, selectedEventTypes, selectedObjectTypes, perspective]);
                            ...
                            <div className="hv-page w-full flex flex-col h-full pb-[80px]">
                                <header className="hv-topbar flex justify-between items-center px-6 py-4 border-b">
                                    ...
                                    <div className="flex items-center gap-4">
                                        {/* Perspective Toggle with Tooltips */}
                                    ...
                                    <main className="hv-board p-4">
                                        {rows.length > 0 ? (
                                            rows.map(([groupKey, entries]) => {
                                                // [Change 1] Removed dynamic color style from row title. It is now always default/black.
                                                return (
                                                    <section
                                                        className="hv-row"
                                                        key={groupKey}
                                                    ...
                                                    >
                                                        <div className="hv-row-title mb-2 font-semibold text-lg">
                                                            {perspective === 'event' ? 'Event: ' : 'Object: '}
                                                            {groupKey}
                                                        </div>
                                                        {/* [Change 2] Fixed Scroll Area: Added w-full and flex w-max to force horizontal scroll */}
