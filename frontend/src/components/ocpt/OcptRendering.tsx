...
//     //                     } as ExtendedOperator;
//     //                 }
//     //                 // 2. Make non-related leaf activities silent
...
//     //                             activity: 'τ',
//     //                             isSilent: true,
//     //                             // Give it the active OT so it renders properly without crashing
//     //                             ots: [{ ot: activeOT }],
//     //                         } as SilentActivity;
//     //                     }
//     //                 }
//     //             });
//     //             // CRITICAL FIX: Do NOT run projectTreeOntoOT here!
//     //             // The Process Forest requires the full structure to remain on screen.
//     //         } else {
//     //             // In Standard Mode, we use your original tree pruning algorithm
...
//         if (renderedTree && !originalRenderedTree) {
//             const clonedTree = cloneDeep(renderedTree);
//             // Only run the legacy extended operator updater in Normal Mode
//             if (!isForestMode) {
//                 updateTreeWithExtendedOperators(clonedTree);
//             }
//             setOriginalRenderedOcpt(clonedTree);
...
//             setRenderedOcpt(clonedTree);
//         }
//     }, [renderedTree, originalRenderedTree, setOriginalRenderedOcpt, setRenderedOcpt, isForestMode]);
...
//             if (isForestMode) {
//                 const activeOT = filteredObjectTypes[0];
//                 // Process Forest: Swap icons and inject τ (Tau) nodes
//                 newTree.each((node) => {
//                     const val = node.data.value;
//                     if (!val) return;
//                     // Swap the operator based on the active object type
//                     if ('operators' in val) {
//                         const projectedOperator = val.operators[activeOT] || 'parallel';
//                         node.data.value = {
//                             operator: projectedOperator,
...
//                 });
//             } else {
//                 // Normal Mode: Prune branches completely
//                 projectTreeOntoOT(newTree, filteredObjectTypes);
//             }
//         } else {
//             newTree = originalRenderedTree;
...
//     const { setRenderedOcpt } = useRenderedOcpt();
//     const { setOriginalRenderedOcpt } = useOriginalRenderedOcpt();
//     // 1. INITIAL SETUP: Save the PURE, un-mutated tree from the backend/mock data.
//     // Do NOT run updateTreeWithExtendedOperators here, or it will destroy the Process Forest operators!
//     useEffect(() => {
//         if (renderedTree && !originalRenderedTree) {
...
//     // 2. PROJECTION ENGINE: Dynamically rebuild the tree every time a toggle is clicked
//     useEffect(() => {
//         if (!originalRenderedTree) return;
...
//             if (isForestMode) {
//                 const activeOT = filteredObjectTypes[0];
//                 // FOREST MODE: Apply dynamic swaps
//                 newTree.each((node) => {
//                     const val = node.data.value;
//                     if (!val) return;
...
//                         // Just pick the first available operator as a fallback
